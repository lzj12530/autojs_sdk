...
// let entry = textContains('机场云商').findOne(1000)
...
//     let point = textContains('申请门店').findOne(1000).bounds()
//     let x = point.centerX() + 500, y =  point.centerY()
//     while(true) {
//         // 刷门店
//         click(x,y)
...
//     }
// }
let btn = text('积分门店').findOne(1000)
if (text('积分门店').findOne(1000)) {
    click_item('积分门店', [20, - 100], '找到入口')
    sleep(800)
    let bounds = btn.bounds()
    let agree = text('我已阅读并同意').findOne(500)
    let times = Math.floor(agree.bounds().top / 1000)
    console.log(agree.bounds(), times)
    let y = 1000
    while(times--) {
        gesture(200, [200, y], [200, y - 800])
        y += 1000
        sleep(200)
    }
    // gesture(200, [200, 1000], [200,200])
}

// gesture(200, btn.bounds())